...
class XoopsCare
{
	...
	function XoopsCare($force = false) {
		global $xoopsDB;
		$this->db = $xoopsDB;
		$this->force = $force;
	...
	function saveLog()
	{
		if(function_exists('file_put_contents')) {
			...
			file_put_contents($this->logfile, join("\n",$this->logdata)."\n", FILE_APPEND);
		} else {
			$fp = null;
			if(!file_exists($this->logfile)) {
	...
	function _fetchLogResult(&$result)
	{
	...
	function ClearComments()
	{
		...
		if( $this->force || ( ($every > 0) && ( (time() - $last_time) > ($every * 86400) ) )) {
			...
			if(xoopscare_getmoduleoption('spamstate') != 1) {	// If we don't keep spammed comments
				...
				$critere->add(new Criteria('com_title', '%'.$replacement.'%', 'LIKE'));
				$critere->add(new Criteria('com_text', '%'.$replacement.'%', 'LIKE'),'OR');
				$tbl_comments = array();
				$tbl_comments = $comment_handler->getObjects($critere);
	...
	function cleanFastestCacheHack()
	{
